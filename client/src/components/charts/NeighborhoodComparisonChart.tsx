import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from 'recharts';
import { useChartData } from '../../hooks/useChartData';
import { getNeighborhoodComparison } from '../../lib/api';
import { getNeighborhoodColor } from '../../lib/utils';
import type { Filters } from '../../lib/types';

interface Props {
  filters: Filters;
}

export function NeighborhoodComparisonChart({ filters }: Props) {
  const { data, loading, error } = useChartData(
    () => getNeighborhoodComparison(filters),
    [JSON.stringify(filters)]
  );

  if (loading) return <div className="h-64 flex items-center justify-center text-slate-400">Loading...</div>;
  if (error) return <div className="h-64 flex items-center justify-center text-red-500">{error}</div>;
  if (!data?.length) return <div className="h-64 flex items-center justify-center text-slate-400">No data</div>;

  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} layout="vertical" margin={{ left: 80, right: 20, top: 10, bottom: 10 }}>
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis type="number" domain={[0, 'auto']} />
        <YAxis type="category" dataKey="neighborhood_display" width={80} tick={{ fontSize: 12 }} />
        <Tooltip formatter={(value) => [Number(value).toFixed(3) + 'x', 'Avg Congestion']} />
        <Bar dataKey="avg_congestion" radius={[0, 4, 4, 0]}>
          {data.map((entry) => (
            <Cell key={entry.neighborhood} fill={getNeighborhoodColor(entry.neighborhood)} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
}
